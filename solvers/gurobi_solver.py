...
class GurobiSolver(BaseSolver):
    """Gurobi solver using the generic Pyomo model."""

    def __init__(self, license_path=None, log_file=None):
        super().__init__("Gurobi_Exact")
    ...
    def solve(self, instance_data, time_limit=300.0):
        ...
        try:
            ...
            # Solve with Gurobi
            solution = model.solve(time_limit=time_limit, solver_name='gurobi')
            if solution is None:
            ...
            # Write diagnostics (same format as original)
            diagnostics_dir = os.path.join(os.getcwd(), "diagnostics")
            os.makedirs(diagnostics_dir, exist_ok=True)
            model._write_diagnostics(solution, diagnostics_dir, instance_data)
    ...
    def get_solver_stats(self):
        stats = {
            ...
            'solver_version': 'Gurobi via Pyomo',
