...
Equations (1)-(35) implemented exactly, with the corrected AND linearization for Z.
Supports Gurobi, MOSEK, HiGHS via Pyomo's SolverFactory.
Output format and diagnostics are identical to the original Gurobi code.
"""
...
class IntegratedCrewSchedulingModel:
    """
    Pyomo implementation of Saemi et al. (2021) integrated crew scheduling model.
    All equations (1)-(35) are implemented according to the TeX formulation,
    with the standard AND linearization for Z variables.
    ...
    def __init__(self, instance_data, log_file=None):
        ...
        self.I = list(range(self.num_crew))                     # 0‑based crew indices
        self.D = list(range(1, self.num_days + 1))              # 1‑based days
    ...
    # ------------------------------------------------------------------
    # Data preprocessing (identical to original)
    # ------------------------------------------------------------------
    def _precompute_flights_and_costs(self):
        ...
        for f in self.instance.get("flights", []):
            ...
            dep_key = f.get("departure_time", f.get("depart"))
            arr_key = f.get("arrival_time", f.get("arrive"))
            raw_flights.append((fid, f.get("origin"), f.get("destination"),
                                day_raw, dep_key, arr_key,
                                f.get("deadhead_cost", None), f.get("uncover_cost", None)))

        # Shift days to make minimum day == 1
        if raw_days:
        ...
        # Flights by day and by (origin,dest,day)
    ...
    def _determine_rounds_and_duties(self):
        ...
        self.N_minus_1 = list(range(1, max_rounds))  # for Z
    ...
    def _build_pyomo_model(self):
        ...
        self.model.x = pyo.Var(self.model.I, self.model.Flights, self.model.N,
                               within=pyo.Binary)
        self.var_stats["x"] += len(self.model.x)
    ...
    def _build_var_name_map(self):
        """Populate var_name_map with metadata for each variable."""
        # x
        for i in self.model.I:
            for fid in self.model.Flights:
                for n in self.model.N:
                    ...
                    name = var.name
                    self.var_name_map[name] = {
                        "kind": "x",
                        "crew": i,
                        "fid": fid,
                        "round": n
                    }
        # Z – not needed for output, skip
        ...
        # b
        for fid in self.model.Flights:
            var = self.model.b[fid]
            self.var_name_map[var.name] = {
                "kind": "b",
                "fid": fid
            }
        # y and SS are not needed for output, skip
    ...
    def _add_constraints(self):
            ...
            for i in model.I:
                for (fid1, fid2) in self.same_day_pairs:
                    ...
                    for n in model.N_minus_1:
                        expr = gap >= min_sit - M * (2 - model.x[i, fid2, n+1] - model.x[i, fid1, n])
                        model.add_component(f"c4_{i}_{fid1}_{fid2}_{n}", pyo.Constraint(expr=expr))
        ...
        def c5_build(model):
            for i in model.I:
                for (fid1, fid2) in self.next_day_pairs:
                    ...
                    gap = 1440 + f2["de"] - f1["l"]
                    for n in model.N_minus_1:
                        expr = gap >= min_rest - M * (2 - model.x[i, fid2, n+1] - model.x[i, fid1, n])
                        model.add_component(f"c5_{i}_{fid1}_{fid2}_{n}", pyo.Constraint(expr=expr))
        ...
        # (21) First flight of first duty starts at home
    ...
    def solve(self, time_limit=300.0, solver_name='gurobi'):
        ...
        # Set solver options (common ones)
    ...
    def _extract_solution(self, results, solve_time):
        """Extract all solution information into a dictionary (same format as original)."""
        sol = {
            "feasibility": "Unknown",
        ...
        # If solution is available, extract variable values
        if sol["feasibility"] not in ("Infeasible", "Unbounded", "Error", "Unknown"):
            sol["objective_value"] = pyo.value(self.model.obj)

            # Raw variable values (for diagnostics) – skip uninitialized variables
            for var in self.model.component_data_objects(pyo.Var, descend_into=True):
                ...
                if name.startswith('x['):
                    sol["raw_vars"]["x"][name] = val
                elif name.startswith('v['):
                    sol["raw_vars"]["v"][name] = val
                elif name.startswith('b['):
                    sol["raw_vars"]["b"][name] = val
                elif name.startswith('s['):
                    sol["raw_vars"]["s"][name] = val
                elif name.startswith('w['):
                    sol["raw_vars"]["w"][name] = val
                # else ignore (Z, y, SS)
            ...
            for i in self.model.I:
                for fid in self.model.Flights:
                    for n in self.model.N:
                        try:
                            # Use .value directly, avoid pyo.value which logs errors on uninitialized vars
            ...
            for i, assigns in sol["crew_assignments"].items():
                assigns.sort(key=lambda a: (a["day"], a["departure_time"]))
            ...
            for i in self.model.I:
                ...
                for j in self.model.J:
                    ...
                    for d in self.model.D:
                        ...
                        if wval is not None and wval > 0.5:
                            hotel += 1
    ...
    def _objective_breakdown(self):
        ...
        for fid in self.model.Flights:
            ...
            if vval is None:
                vval = 0.0
            uncover_sum += self.uncover_costs[fid] * (1 - vval)
            bval = self.model.b[fid].value
            if bval is None:
                bval = 0.0
            deadhead_sum += self.flight_costs[fid] * (bval - vval)
        for i in self.model.I:
            for j in self.model.J:
                if j == self.o_i.get(i):
                    continue
                for d in self.model.D:
                    ...
                    if wval is None:
                        wval = 0.0
                    away_sum += self.h * wval
            SSval = self.model.SS[i].value
            if SSval is None:
                SSval = 0.0
            crew_sum += self.r_i[i] * SSval
        # Compute total from components (always safe and matches objective)
    ...
    # ------------------------------------------------------------------
    # Diagnostics writer (identical to original)
    # ------------------------------------------------------------------
    def _write_diagnostics(self, solution, diagnostics_dir, instance_data):
        """Write the five diagnostic CSV files."""
        ...
        with open(crew_csv, 'w', newline='') as f:
            ...
            num_crew = instance_data.get("parameters", {}).get("num_crew", 0)
            crew_assignments = solution.get("crew_assignments", {})
            for i in range(num_crew):
                home = instance_data.get("crew", [{}]*num_crew)[i].get("home_base", instance_data.get("cities", [""])[0]) if i < len(instance_data.get("crew", [])) else instance_data.get("cities", [""])[0]
        ...
        with open(flights_csv, 'w', newline='') as f:
            ...
            vmap = raw.get("v", {})
            bmap = raw.get("b", {})
            for fl in instance_data.get("flights", []):
                fid = fl["flight_id"]
                writer.writerow([
                    fid,
                    fl.get("origin", ""),
        ...
        with open(vars_csv, 'w', newline='') as f:
            ...
            raw = solution.get("raw_vars", {})
            for k, v in raw.get("x", {}).items():
                writer.writerow([f"x_{k}", v])
            for k, v in raw.get("v", {}).items():
                writer.writerow([f"v_{k}", v])
            for k, v in raw.get("b", {}).items():
                writer.writerow([f"b_{k}", v])
            for k, v in raw.get("s", {}).items():
                writer.writerow([f"s_{k}", v])
            for k, v in raw.get("w", {}).items():
        ...
        with open(mismatch_csv, 'w', newline='') as f:
            ...
            for i in range(num_crew):
                ...
                for j in cities:
                    for d in range(instance_data.get("parameters", {}).get("num_days", 0)):
                        inferred_s = 0
                        inferred_w = 0
                        if per_day.get(d):
        ...
        with open(viol_csv, 'w', newline='') as f:
            ...
            violations = solution.get("flow_violations", [])
            for v in violations:
